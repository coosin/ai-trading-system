...
echo "== [3/6] wait /health (host -> published port) =="
ok=0
for i in $(seq 1 50); do
  if curl -sf --max-time 15 "http://127.0.0.1:${API_PORT:-8000}/health" >/dev/null; then
    ok=1
...
docker compose exec -T trading-system python3 scripts/network_connectivity_smoke.py \
  --redis \
  --api-url "http://127.0.0.1:8000/health"
...
echo "== [6/6] commander mirror audit (optional) =="
curl -sf --max-time 60 "http://127.0.0.1:${API_PORT:-8000}/api/v1/commander/_audit" | head -c 2500 || echo "(audit skip or non-200)"
