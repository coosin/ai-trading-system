...
def _get_json(url: str, timeout: float) -> Tuple[int, str]:
...
def main() -> int:
    ...
    try:
        code, body = _post_json(dispatch_url, sync_payload, timeout=max(2.0, args.sync_timeout_sec + 2.0))
    ...
    try:
        code, body = _post_json(dispatch_url, async_payload, timeout=10.0)
        data = _parse_json(body)
    ...
    while time.time() < deadline:
        try:
            _, body = _get_json(job_url, timeout=10.0)
            data = _parse_json(body)
