...
- **仓库**：`agents/`（本地会话类 `*.jsonl` 等）不再纳入版本控制，已加入 `.gitignore` 并从索引移除历史误提交文件；`workspace/memory/working/` 等运行时碎片与 `data/**/*.db`、`logs/`、`backups/` 等写入 `.gitignore`，已跟踪的 `working/*.json` 从索引移除。
