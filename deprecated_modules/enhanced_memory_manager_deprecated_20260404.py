...
class EnhancedMemoryManager:
    ...
    def _extract_important_info(self, content: str) -> Optional[Tuple[MemoryCategory, str]]:
        ...
        for info_type, patterns in self.IMPORTANT_PATTERNS.items():
            for pattern in patterns:
                ...
                if match:
                    ...
                    if self._is_template_content(extracted):
                        continue
        ...
        for keyword in important_keywords:
            if keyword in content and len(content) < 200:
                if not self._is_template_content(content):
    ...
    def _is_template_content(self, content: str) -> bool:
        ...
        if content.startswith("分析以下") or content.startswith("请提供"):
            return True
        
        if re.search(r"^\d+\.\s*(?:市场|关键|技术|交易)", content):
            return True
