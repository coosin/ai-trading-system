...
class RiskManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        self.config_path = config_path or os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "crypto-config", "risk_config.json"
        )
    ...
    def load_config(self) -> Dict:
        ...
        default_config = {
            ...
            "portfolio": {"total_capital": 10000.0, "base_currency": "USDT"},  # 默认总资金
            "risk_factors": {
    ...
    def generate_risk_report(self) -> str:
        ...
        # 总体风险评估
        critical_count = sum(
            1 for r in position_risks if r["risk_level"] == RiskLevel.CRITICAL.value
        )
        if critical_count > 0:
        ...
        if pnl_percent < -self.config["risk_limits"]["max_daily_loss_percent"]:
            report_lines.append(
                f"🚨 紧急: 已达到每日亏损限额 ({self.config['risk_limits']['max_daily_loss_percent']}%)"
            )
