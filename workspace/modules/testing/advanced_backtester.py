...
class AdvancedBacktester:
    ...
    async def optimize_parameters(
        self,
        strategy: Callable,
        param_grids: List[ParameterGrid],
        start_date: datetime,
        end_date: datetime,
    ...
    async def monte_carlo_simulation(
        ...
        # 获取基准回测结果
        baseline_result = await self.backtest_strategy(strategy, parameters, start_date, end_date)
    ...
    async def stress_test(
        self, strategy: Callable, parameters: Dict[str, Any], scenarios: List[str] = None
        ...
        for scenario in scenarios:
            ...
            # 回测
            result = await self.backtest_strategy_with_scenario(strategy, parameters, scenario_data)
    ...
    def _update_state(
        self, state: Dict, current_data: pd.Series, previous_data: pd.Series
    ) -> Dict[str, Any]:
        """更新状态"""
        ...
        if trade_type == "buy":
            ...
            if state["position"] >= quantity:
                ...
                if state["entry_price"] > 0:
                    ...
                    trade["pnl_percent"] = (execution_price - state["entry_price"]) / state[
                        "entry_price"
                    ]
            else:
                return None  # 持仓不足
    ...
    async def _grid_search_optimization(
        ...
        for i, params in enumerate(combinations):
            ...
            result = await self.backtest_strategy(strategy, params, start_date, end_date)
    ...
    def _analyze_monte_carlo_results(
        self, baseline: BacktestResult, simulations: List[BacktestResult], confidence_level: float
    ) -> Dict[str, Any]:
        """分析蒙特卡洛模拟结果"""
    ...
    async def backtest_strategy_with_scenario(
        self, strategy: Callable, parameters: Dict[str, Any], scenario_data: Dict
        ...
        return await self.backtest_strategy(
            strategy, parameters, datetime.now() - timedelta(days=90), datetime.now()
        )
...
def get_backtester(
    data_provider=None, commission_rate: float = 0.001, slippage: float = 0.001
) -> AdvancedBacktester:
    """获取回测器单例"""
    global _backtester
