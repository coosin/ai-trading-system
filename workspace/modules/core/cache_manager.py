...
class SmartCacheManager:
    ...
    async def set(self, key: str, value: Any, ttl_seconds: int = None, 
                 metadata: Dict[str, Any] = None, compress: bool = None):
        """设置缓存值"""
    ...
    async def get_or_set(self, key: str, callback: Callable, ttl_seconds: int = None,
                        *args, **kwargs) -> Any:
        """获取或设置缓存值（缓存穿透防护）"""
    ...
    async def _set_to_redis(self, key: str, value: Any, ttl_seconds: int):
        ...
        try:
            # 压缩
            value_to_store = self._compress(value) if isinstance(value, (bytes, str, dict, list)) else value
        ...
        while self.is_running:
            try:
                ...
                if len(self.stats_history) % 5 == 0:  # 每5次打印一次
                    print(f"📊 缓存统计: {stats.total_entries} 条目, "
