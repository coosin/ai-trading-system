...
class DynamicConfigManager:
    ...
    def _get_default_config(self, config_type: str) -> Dict[str, Any]:
        ...
        default_configs = {
            ...
            "risk": {
                ...
                "risk_limits": {"daily": 2.0, "weekly": 5.0, "monthly": 10.0},
            ...
            "data_sources": {
                "market_data": {
                    ...
                    "coinbase": {"enabled": False, "api_key": "", "api_secret": ""},
                },
                "onchain_data": {
                    "glassnode": {"enabled": True, "api_key": ""},
                    "cryptoquant": {"enabled": True, "api_key": ""},
                },
                "sentiment_data": {
        ...
        try:
            ...
            if config_type:
                ...
                self._trigger_change_callbacks(
                    config_type, old_config, self._config_store[config_type]
                )
    ...
    def set_config(self, config_type: str, key: str, value: Any):
        ...
        # 触发变更回调
        if old_value != value:
            self._trigger_change_callbacks(config_type, {key: old_value}, {key: value})
    ...
    def get_config_history(
        self, config_type: str = None, limit: int = 50
    ) -> List[ConfigChangeEvent]:
        """获取配置变更历史"""

        if config_type:
            filtered = [event for event in self._config_history if event.config_type == config_type]
        else:
    ...
    def _validate_trading_config(self, config: Dict) -> List[str]:
        ...
        if "max_position_size_percent" in config:
            if (
                config["max_position_size_percent"] <= 0
                or config["max_position_size_percent"] > 100
            ):
                errors.append(
                    f"无效的 max_position_size_percent: {config['max_position_size_percent']}"
                )
    ...
    try:
        ...
        def on_system_change(config_type, old_config, new_config):
            ...
            if "log_level" in new_config:
                print(
                    f"日志级别从 {old_config.get('log_level')} 变为 {new_config.get('log_level')}"
                )
