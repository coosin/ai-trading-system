...
class ExecutionPlan:
    ...
    def calculate_metrics(self, market_state: MarketState):
        ...
        self.estimated_slippage = (
            total_slippage / (self.total_quantity * market_state.mid_price)
            if self.total_quantity > 0
            else 0.0
        )
        self.market_impact = (
            sum(order.get("market_impact", 0) for order in self.child_orders)
            / len(self.child_orders)
...
class ExecutionResult:
    ...
    def calculate_quality_score(self) -> float:
        ...
        if self.total_cost > 0:
            cost_score = max(
                0, 100 - (self.total_cost / self.total_quantity / self.average_price * 10000)
            )
        else:
            cost_score = 100
        ...
        # 综合分数
        weights = {"completion": 0.3, "cost": 0.3, "slippage": 0.2, "speed": 0.2}
...
class ExecutionOptimizer:
            ...
            self.config.update(
                {
                    ...
                    "max_market_impact_percent": execution_config.get(
                        "max_market_impact_percent", 5.0
                    ),
                    "default_time_horizon_minutes": execution_config.get(
                        "default_time_horizon_minutes", 60
                    ),
                    "volume_participation_rate": execution_config.get(
    ...
    async def create_execution_plan(self, order_request: Dict) -> ExecutionPlan:
        ...
        if optimization_algorithm in self.optimization_algorithms:
            ...
            # 默认使用自适应执行
            child_orders = await self._optimize_adaptive(
                symbol, side, quantity, current_market, urgency
            )
            for order in child_orders:
                plan.add_child_order(order)
    ...
    def _select_optimization_algorithm(
        self, order_type: str, urgency: str, quantity: float, market_state: MarketState
    ) -> str:
        """选择优化算法"""
    ...
    async def _optimize_twap(
        self, symbol: str, side: str, quantity: float, market_state: MarketState, urgency: str
    ) -> List[Dict]:
        """TWAP（时间加权平均价格）优化"""
    ...
    async def _optimize_adaptive(
        ...
        if market_condition == "high_volatility":
            # 高波动市场，使用小订单快速执行
            return await self._optimize_high_volatility(
                symbol, side, quantity, market_state, urgency
            )
    ...
    async def _optimize_high_volatility(
        self, symbol: str, side: str, quantity: float, market_state: MarketState, urgency: str
    ) -> List[Dict]:
        """高波动市场优化"""
    ...
    async def _optimize_low_liquidity(
        self, symbol: str, side: str, quantity: float, market_state: MarketState, urgency: str
    ) -> List[Dict]:
        """低流动性市场优化"""
    ...
    async def _optimize_trending(
        ...
        if (side == "BUY" and trend_direction == "up") or (
            side == "SELL" and trend_direction == "down"
        ):
            # 顺势交易，可以更积极
            print(f"   📈 顺势交易，使用积极执行")
    ...
    async def _optimize_normal(
        ...
        if twap_ratio > 0:
            ...
            twap_orders = await self._optimize_twap(
                symbol, side, twap_quantity, market_state, urgency
            )
        ...
        if vwap_ratio > 0:
            ...
            vwap_orders = await self._optimize_vwap(
                symbol, side, vwap_quantity, market_state, urgency
            )
    ...
    def _calculate_order_count(
        self, quantity: float, market_state: MarketState, urgency: str
    ) -> int:
        """计算订单拆分数"""
        ...
        base_count = {"urgent": 3, "high": 5, "normal": 10, "low": 20}.get(urgency, 10)
    ...
    async def _apply_learning_suggestions(
        self, plan: ExecutionPlan, market_state: MarketState
    ) -> ExecutionPlan:
        """应用学习模型建议"""
        ...
        suggestions = self.learning_model.get_suggestions(
            plan.symbol, plan.total_quantity, market_state, plan.order_type
        )
    ...
    def _calculate_confidence_score(self, plan: ExecutionPlan, market_state: MarketState) -> float:
        ...
        # 综合分数
        weights = {"condition": 0.3, "size": 0.25, "strategy": 0.25, "history": 0.2}
    ...
    def get_performance_report(self, symbol: str = None, days: int = 30) -> Dict[str, Any]:
        ...
        if symbol:
            filtered_results = [
                r for r in self.execution_history if r.symbol == symbol and r.end_time > cutoff_date
            ]
        else:
            filtered_results = [r for r in self.execution_history if r.end_time > cutoff_date]

        if not filtered_results:
            return {"total_executions": 0, "message": "无执行记录"}
        ...
        report = {
            ...
            "top_performing_strategies": sorted(
                strategies.items(), key=lambda x: x[1]["avg_quality"], reverse=True
            )[:3],
        }
...
class ExecutionLearningModel:
    ...
    def get_suggestions(
        self, symbol: str, quantity: float, market_state: MarketState, strategy: str
    ) -> Dict[str, float]:
        """获取优化建议"""
...
async def test_execution_optimizer():
    ...
    market_state = MarketState(
        ...
        volume_24h=1000000.0,
        volatility=0.02,
        order_book_depth={"bid_0.1%": 10.0, "bid_0.5%": 50.0, "ask_0.1%": 10.0, "ask_0.5%": 50.0},
    )
    ...
    for i, order in enumerate(plan.child_orders[:3]):  # 只显示前3个
        print(
            f"   订单{i+1}: {order.get('quantity', 0):.4f} {order.get('order_type', 'N/A')} "
