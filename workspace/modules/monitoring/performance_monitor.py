...
class PerformanceMonitor:
        ...
        while self.is_running:
            try:
                ...
                await self.record_metric(
                    "memory", "usage_percent", memory.percent, {"type": "system"}
                )
                await self.record_metric(
                    "memory", "used_mb", memory.used / 1024 / 1024, {"type": "system"}
                )
                await self.record_metric(
                    "memory", "available_mb", memory.available / 1024 / 1024, {"type": "system"}
                )
                ...
                await self.record_metric(
                    "disk", "free_gb", disk.free / 1024 / 1024 / 1024, {"type": "system"}
                )

                # 网络IO
                net_io = psutil.net_io_counters()
                await self.record_metric(
                    "network", "bytes_sent", net_io.bytes_sent, {"type": "system"}
                )
                await self.record_metric(
                    "network", "bytes_recv", net_io.bytes_recv, {"type": "system"}
                )
    ...
    async def _monitor_application_metrics(self):
        ...
        while self.is_running:
            try:
                ...
                await self.record_metric(
                    "process", "rss_mb", process_memory.rss / 1024 / 1024, {"type": "application"}
                )
                await self.record_metric(
                    "process", "vms_mb", process_memory.vms / 1024 / 1024, {"type": "application"}
                )

                # 进程CPU
                process_cpu = process.cpu_percent()
                await self.record_metric(
                    "process", "cpu_percent", process_cpu, {"type": "application"}
                )

                # 线程数
                thread_count = process.num_threads()
                await self.record_metric(
                    "process", "thread_count", thread_count, {"type": "application"}
                )
    ...
    async def record_metric(
        self, category: str, metric_name: str, value: float, tags: Dict[str, str] = None
    ):
        """记录性能指标"""
        ...
        for metric_type, metrics in self.metric_windows.items():
            ...
            if is_bottleneck:
                ...
                recommendations = self._generate_recommendations(
                    metric_type, current_value, avg_value
                )
    ...
    def _generate_recommendations(
        self, metric_type: str, current_value: float, avg_value: float
    ) -> List[str]:
        """生成优化建议"""
    ...
    def export_metrics(self, format: str = "json") -> str:
        ...
        data = {
            ...
            "recent_bottlenecks": [
                asdict(bottleneck) for bottleneck in self.get_bottlenecks(limit=10)
            ],
        }
    ...
    async def profile_function(self, func_name: str, func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            # 记录延迟指标
            await self.record_metric("latency", func_name, latency, {"function": func_name})
            ...
            # 记录错误
            await self.record_metric(
                "error", f"{func_name}_error", 1.0, {"function": func_name, "error": str(e)}
            )
            raise e
