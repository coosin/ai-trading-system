...
class RiskAwareModel(BaseTradingModel):
    ...
    def _calculate_risk_score(
        self, volatility: float, position_size: float, max_position: float
    ) -> float:
        """计算风险得分"""
...
class MultiModelDecisionEngine:
    ...
    def __init__(self):
        ...
        # 模型权重（可动态调整）
        self.model_weights = {"technical": 1.0, "sentiment": 0.8, "risk": 1.2}

        # 决策历史
        self.decision_history = []

    async def analyze_market(
        self, symbol: str, market_data: Dict, portfolio: Dict = None
