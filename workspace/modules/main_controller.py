...
from .core.config_manager import RiskConfig, SystemConfig, TradingConfig, get_config_manager
from .core.data_pipeline import MarketData, OrderBookSnapshot, get_data_pipeline
...
class MainController:
    ...
    def __init__(self):
        ...
        self.order_manager = get_order_manager(
            None, self.config_manager
        )  # 需要实际的exchange_adapter
    ...
    async def start(self, mode: str = "paper_trading"):
        ...
        try:
            ...
            await self._trigger_event(
                "system_started", {"mode": mode, "timestamp": datetime.now().isoformat()}
            )
    ...
    async def _execute_trade_signal(self, signal: TradingSignal):
        ...
        try:
            ...
            order = await self.order_manager.create_order(
                trade_instruction, self.portfolio.to_dict()
            )
            ...
            if order:
                ...
                print(
                    f"📝 创建订单: {order.order_id} {order.side} {order.quantity} {order.symbol} @ {order.price}"
                )
    ...
    def _calculate_position_size(self, signal: TradingSignal) -> float:
        ...
        # 计算最终仓位比例
        position_percent = (
            base_position_percent * position_multiplier * confidence_multiplier * risk_multiplier
        )
