...
if [ -d "$HOME/.openclaw-trading" ]; then
    echo "   ✅ 存在"
    echo "      配置文件: $(ls -la ~/.openclaw-trading/openclaw-trading.json 2>/dev/null | wc -l) 个"
...
echo "   交易系统提权权限:"
TRADING_ELEVATED=$(grep -A5 '"elevated"' ~/.openclaw-trading/openclaw-trading.json 2>/dev/null | grep '"enabled"' | grep -o 'true\|false')
if [ "$TRADING_ELEVATED" = "true" ]; then
    echo "   ⚠️  已启用提权权限"
else
    echo "   ✅ 提权权限已禁用"
fi
