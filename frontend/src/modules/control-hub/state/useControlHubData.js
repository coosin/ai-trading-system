...
export function useControlHubData() {
  ...
  const [state, setState] = useState({
    ...
    memorySummary: [],
    memoryDailySummaryRaw: null,
    memoryStats: null,
    commanderSnapshot: null,
    systemMastery: null,
    platformOversight: null,
    ...
    profitOpsOverview: null,
    tradingWorkflow: null,
  ...
  const loadSlow = useCallback(async () => {
    ...
    markError('slow', null);
    try {
      const [acceptance, accountDiagnostics, executionSpine, tradeHistory, strategies, strategyOpt, researchJobs, productionAudit, memorySummary, memoryStats, commanderSnapshot, tradingWorkflow, systemMastery, platformOversight, closedLoopSummary, agentEffectiveness, commanderAudit, commanderCapabilities, hostingMode, hostingGuard, automationProfile, riskRedlines, toolContract, governanceAudit, aiGuards, stopLossStats, tradeEvents, surfaceRegistry, dataIntegrationHealth, pluginsStatus, opportunities, profitAnalytics, profitOpsOverview, researchCockpit, weeklyReview, retrievalDeck] =
        await Promise.all([
          ...
          api.modules.getMemoryStats().catch(() => null),
          api.modules.getCommanderSnapshot(symbol, 'fast').catch(() => null),
          api.modules.getCommanderTradingWorkflow(symbol).catch(() => null),
          api.modules.getCommanderSystemMastery(symbol).catch(() => null),
          api.modules.getCommanderPlatformOversight(symbol, { trace_limit: 120, trade_limit: 300 }).catch(() => null),
          api.modules.getCommanderClosedLoopSummary({ trace_limit: 120 }).catch(() => null),
          api.modules.getCommanderAgentEffectiveness({ trace_limit: 120, trade_limit: 300 }).catch(() => null),
          ...
          api.modules.getProfitOpsOverview({ days: 30, sample_limit: 200, active_order_limit: 20 }).catch(() => null),
      ...
      updateSlice({
        ...
        memoryDailySummaryRaw: unwrap(memorySummary),
        memoryStats: unwrap(memoryStats),
        commanderSnapshot: unwrap(commanderSnapshot),
        tradingWorkflow: unwrap(tradingWorkflow),
        systemMastery: unwrap(systemMastery),
        platformOversight: unwrap(platformOversight),
        closedLoopSummary: unwrap(closedLoopSummary),
        agentEffectiveness: unwrap(agentEffectiveness),
        ...
        profitOpsOverview: unwrap(profitOpsOverview),
