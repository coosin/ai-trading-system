...
class StrategyManager:
    ...
    async def register_strategy_class(self, class_name: str, strategy_class: Type[BaseStrategy]) -> bool:
        """
        注册策略类
    ...
    async def create_strategy_instance(self, strategy_id: str, 
    ...
    async def get_strategy_instances(self, strategy_id: Optional[str] = None,
                                   status: Optional[StrategyStatus] = None) -> List[StrategyInstance]:
        """
        获取策略实例列表
    ...
    async def get_signals(self, strategy_id: Optional[str] = None,
    ...
    async def update_strategy_parameters(self, instance_id: str, 
    ...
    async def export_strategy_config(self, strategy_id: str, 
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            running_instances = len([i for i in self.strategy_instances.values() 
                                   if i.status == StrategyStatus.RUNNING])
            ...
            return {
                ...
                "paused_instances": len([i for i in self.strategy_instances.values() 
                                       if i.status == StrategyStatus.PAUSED]),
                "stopped_instances": len([i for i in self.strategy_instances.values() 
                                        if i.status == StrategyStatus.STOPPED]),
                "total_signals": total_signals,
                "signals_last_24h": len([s for s in self.signal_history 
    ...
    async def _check_strategy_health(self) -> None:
        ...
        for instance in self.strategy_instances.values():
            if (instance.status == StrategyStatus.RUNNING and 
                instance.last_signal_at and 
                datetime.now() - instance.last_signal_at > timedelta(hours=1)):
                
                logger.warning(f"策略长时间未生成信号: {instance.instance_id} "
    ...
    async def _calculate_all_performance(self) -> None:
        ...
        for strategy_id in self.strategy_configs:
            if strategy_id not in self.performance_metrics:
                # 创建新的性能记录
                self.performance_metrics[strategy_id] = StrategyPerformance(
                    strategy_id=strategy_id
                )
...
async def example_usage():
    ...
    try:
        # 注册策略类
        await strategy_manager.register_strategy_class(
        ...
        # 获取策略配置
        strategy_configs = list(strategy_manager.strategy_configs.values())
        print(f"可用策略: {len(strategy_configs)} 个")
        
        for config in strategy_configs:
            print(f"  - {config.name} ({config.strategy_type.value}): {config.description}")
        
        # 创建策略实例
        if strategy_configs:
            strategy_id = strategy_configs[0].strategy_id
            ...
            if instance_id:
                ...
                if success:
                    ...
                    # 启动策略
                    start_success = await strategy_manager.start_strategy(instance_id)
                    if start_success:
                        print("策略启动成功")
                        ...
                        signals = await strategy_manager.process_market_data(
                        ...
                        if instance:
                            print(f"策略状态: {instance.status.value}")
                            print(f"总信号数: {instance.total_signals}")
                        ...
                        # 停止策略
                        await strategy_manager.stop_strategy(instance_id)
                        print("策略已停止")
        
        # 导出策略配置
        if strategy_configs:
            config_json = await strategy_manager.export_strategy_config(
                strategy_configs[0].strategy_id, "json"
            )
            if config_json:
                print(f"策略配置JSON:\n{config_json[:200]}...")
