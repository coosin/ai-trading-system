...
class LogEntry:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "LogEntry":
        ...
        return cls(
            ...
            level=LogLevel(data["level"]) if isinstance(data.get("level"), str) else data.get("level", LogLevel.INFO),
            source=LogSource(data["source"]) if isinstance(data.get("source"), str) else data.get("source", LogSource.SYSTEM),
...
class FileHandler(LogHandler):
    ...
    async def _cleanup_old_backups(self) -> None:
        ...
        try:
            ...
            # 删除超出限制的文件
            for i, log_file in enumerate(log_files[self.config.max_backup_count:], self.config.max_backup_count):
                try:
                    log_file.unlink()
...
class LogManager:
    ...
    async def log(self, level: LogLevel, source: LogSource, message: str,
                 module: Optional[str] = None, function: Optional[str] = None,
                 line_no: Optional[int] = None, correlation_id: Optional[str] = None,
                 ...
                 include_traceback: Optional[bool] = None) -> str:
        """
        记录日志
        ...
        # 添加堆栈跟踪
        if (include_traceback or (include_traceback is None and self.config.include_traceback)) and level in [LogLevel.ERROR, LogLevel.CRITICAL]:
            try:
    ...
    async def query_logs(self, query: LogQuery) -> List[LogEntry]:
        ...
        with self._lock:
            ...
            if query.order_by == "timestamp":
                ...
                level_order = {LogLevel.CRITICAL: 5, LogLevel.ERROR: 4, 
                              LogLevel.WARNING: 3, LogLevel.INFO: 2, LogLevel.DEBUG: 1}
                filtered_logs.sort(key=lambda x: level_order.get(x.level, 0), reverse=query.order_desc)
    ...
    async def get_log_statistics(self, start_time: Optional[datetime] = None,
        ...
        with self._lock:
            ...
            # 计算错误率
            error_count = stats.logs_by_level.get(LogLevel.ERROR.value, 0) + stats.logs_by_level.get(LogLevel.CRITICAL.value, 0)
    ...
    async def _load_config(self) -> None:
        ...
        if self.config_manager:
            ...
            if log_config:
                ...
                # 其他配置
                config_fields = ["log_dir", "max_file_size_mb", "max_backup_count", 
                               "rotation_interval", "compress_backups", "json_format",
                               "include_traceback", "buffer_size", "flush_interval_seconds"]
                
                for field in config_fields:
                    if field in log_config:
                        setattr(self.config, field, log_config[field])
        
        logger.info(f"加载日志配置: 级别={self.config.log_level.value}, 输出={[o.value for o in self.config.outputs]}")
    ...
    async def _is_level_enabled(self, level: LogLevel) -> bool:
        """检查日志级别是否启用"""
        level_order = {LogLevel.DEBUG: 1, LogLevel.INFO: 2, LogLevel.WARNING: 3,
    ...
    async def _update_stats(self, entry: LogEntry) -> None:
        ...
        with self._lock:
            ...
            self.stats["logs_by_level"][entry.level.value] = self.stats["logs_by_level"].get(entry.level.value, 0) + 1
            self.stats["logs_by_source"][entry.source.value] = self.stats["logs_by_source"].get(entry.source.value, 0) + 1
...
async def example_usage():
    ...
    try:
        ...
        for log in recent_logs:
            print(f"  [{log.timestamp.strftime('%H:%M:%S')}] {log.level.value.upper()}: {log.message}")
