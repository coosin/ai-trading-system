...
class RiskManager:
    ...
    async def get_alerts(self, severity: Optional[AlertSeverity] = None,
                        unresolved: bool = True) -> List[RiskAlert]:
        """
        获取警报
    ...
    async def _load_config(self) -> None:
        ...
        logger.info(f"加载风险配置: 单仓位限制={self.position_limits.max_position_size*100:.1f}%, "
    ...
    async def _setup_default_rules(self) -> None:
        ...
        position_rule = RiskRule(
            ...
            condition=lambda ctx: ctx.get("position_size_percent", 0) > self.position_limits.max_position_size,
        ...
        daily_loss_rule = RiskRule(
            ...
            condition=lambda ctx: ctx.get("daily_loss_percent", 0) > self.loss_limits.max_daily_loss,
    ...
    async def _check_position_limits(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if position_size_percent > self.position_limits.max_position_size * 0.8:
            return {
                "passed": True,
                "warning": f"仓位比例较高: {position_size_percent*100:.1f}%"
            }
    ...
    async def _check_loss_limits(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if daily_loss_percent > self.loss_limits.max_daily_loss * 0.8:
            return {
                "passed": True,
                "warning": f"接近单日亏损限制: {daily_loss_percent*100:.2f}%"
            }
    ...
    async def _check_volatility(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if volatility > 0.05:  # 波动率超过5%
            return {
                "passed": True,
                "warning": f"波动率较高: {volatility*100:.1f}%"
            }
    ...
    async def _create_alert(self, rule_id: str, severity: AlertSeverity,
        ...
        logger.log(
            ...
            logging.WARNING if severity == AlertSeverity.ERROR else
    ...
    async def _metrics_calculation_worker(self) -> None:
        ...
        while self._initialized:
            try:
                ...
                logger.debug(f"风险指标更新: VaR95=${var_95:.2f}, 最大回撤={max_drawdown*100:.1f}%, 夏普比率={sharpe_ratio:.2f}")
    ...
    async def _calculate_risk_level(self) -> str:
        ...
        # 活跃警报
        active_critical = len([a for a in self.alerts.values() if a.severity == AlertSeverity.CRITICAL and not a.resolved])
