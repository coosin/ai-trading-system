...
class Order:
    ...
    def update_fill(self, fill_quantity: float, fill_price: float, commission: float = 0.0) -> None:
        ...
        self.avg_fill_price = (
            (self.avg_fill_price * (self.filled_quantity - fill_quantity) + fill_price * fill_quantity) 
...
class Position:
    ...
    def update_position(self, side: OrderSide, quantity: float, price: float, commission: float) -> None:
        """更新仓位"""
        ...
        if side == OrderSide.BUY:
            ...
            if new_quantity > 0:
                self.avg_entry_price = (old_quantity * old_avg_price + quantity * price) / new_quantity
...
class TradeEngine:
    ...
    async def create_order(self, symbol: str, side: OrderSide, order_type: OrderType,
                          ...
                          stop_price: Optional[float] = None,
                          strategy_id: Optional[str] = None) -> Optional[Order]:
        """
        创建订单
            ...
            try:
                ...
                if side == OrderSide.BUY:
                    ...
                    if self.cash_balance - self.reserved_cash >= required_cash:
                        ...
                        logger.error(f"资金不足: 需要${required_cash:,.2f}, 可用${self.cash_balance - self.reserved_cash:,.2f}")
                        return None
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        async with self._lock:
            ...
            if order.side == OrderSide.BUY and order.status == OrderStatus.PENDING:
                order_value = order.quantity * (order.price if order.price else await self._get_current_price(order.symbol))
                required_cash = order_value * (1 + self.commission_rate)
    ...
    async def calculate_pnl(self, symbol: Optional[str] = None,
                           start_time: Optional[datetime] = None,
    ...
    async def _load_config(self) -> None:
        ...
        if self.config_manager:
            ...
            self.max_position_count = trade_config.get("max_position_count", self.max_position_count)
        
        logger.info(f"加载交易配置: 手续费率={self.commission_rate*100:.2f}%, "
    ...
    async def _validate_order(self, symbol: str, side: OrderSide, order_type: OrderType,
        ...
        if symbol in self.positions:
            ...
            if side == OrderSide.BUY and position.side == PositionSide.SHORT:
                ...
                if position.market_value + quantity * (price or await self._get_current_price(symbol)) > max_position_value:
                    return f"超过单仓位限制: 最大${max_position_value:,.2f}"
        
        # 检查持仓数量限制
        active_positions = sum(1 for p in self.positions.values() if abs(p.quantity) > 1e-8)
        if symbol not in self.positions or abs(self.positions.get(symbol, Position(symbol, PositionSide.FLAT)).quantity) < 1e-8:
    ...
    async def _execute_fill(self, order: Order, fill_quantity: float, 
                           fill_price: float, commission: float) -> None:
        """执行成交"""
        async with self._lock:
            # 更新订单
            ...
            logger.info(f"订单成交: {order.id} {order.side.value} {fill_quantity} {order.symbol} "
                       f"@{fill_price:.4f}, 手续费${commission:.4f}")
    
    async def _check_stop_orders(self, symbol: str, current_price: float) -> None:
        """检查止损单"""
        for order_id, order in list(self.orders.items()):
            if (order.symbol == symbol and order.is_active and 
                ...
                if trigger:
                    logger.info(f"止损单触发: {order_id} {order.side.value} {order.symbol} "
                    ...
                    if order.order_type == OrderType.STOP:
                        ...
                        await self._execute_fill(
                            order, order.remaining_quantity, current_price, 
                            max(order.quantity * current_price * self.commission_rate, self.min_commission)
                        )
                        ...
                        if (order.side == OrderSide.BUY and current_price <= order.price) or \
                           (order.side == OrderSide.SELL and current_price >= order.price):
                            ...
                            await self._execute_fill(
                                order, order.remaining_quantity, order.price,
                                max(order.quantity * order.price * self.commission_rate, self.min_commission)
                            )
...
async def example_usage():
    ...
    try:
        ...
        for position in positions:
            print(f"  {position.symbol}: {position.quantity} @ {position.avg_entry_price:.2f}, "
