...
class CacheManager:
                ...
                try:
                    ...
                    if value is not None:
                        ...
                        if cache_level != CacheLevel.MEMORY:
                            await self._set_to_level(full_key, value, CacheLevel.MEMORY, 
    ...
    async def _set_to_level(self, key: str, value: Any, 
                           level: CacheLevel, ttl: int) -> bool:
        """设置缓存到指定级别"""
        if level == CacheLevel.MEMORY:
    ...
    def _evict_from_memory(self) -> None:
        ...
        if self.eviction_policy == EvictionPolicy.LRU:
            ...
            key = min(self.memory_cache.keys(),
                     ...
                     if self.memory_cache[k].expires_at else datetime.max)
            entry = self.memory_cache[key]
        else:  # RANDOM
            # 随机淘汰
            import random
