...
class ProactiveMarketScanner:
    ...
    async def _evaluate_and_execute(self, opportunity: MarketOpportunity) -> bool:
        ...
        # Legacy auto-execute flags are now normalized to forward-only.
        if bool(self._auto_execute_opportunities):
            logger.warning(
                "proactive_auto_execute_opportunities 已废弃为 advisory-only；机会将统一转发给 ai_core: %s",
                opportunity.symbol,
            )
