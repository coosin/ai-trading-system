...
class MonitorManager:
    ...
    async def collect_metric(self, monitor_id: str, name: str, value: float,
    ...
    async def get_metrics(self, monitor_id: Optional[str] = None,
    ...
    async def get_metric_statistics(self, monitor_id: str, name: str,
                                  start_time: Optional[datetime] = None,
    ...
    async def get_alerts(self, monitor_id: Optional[str] = None,
                        alert_level: Optional[AlertLevel] = None,
                        unresolved: bool = True) -> List[MonitorAlert]:
        """
        获取告警
    ...
    async def update_service_health(self, service_name: str, status: ServiceStatus,
                                  response_time: float = 0.0, success_rate: float = 1.0,
    ...
    async def get_system_summary(self) -> Dict[str, Any]:
        ...
        # 计算趋势（最近5个点）
        recent_metrics = self.system_metrics[-5:] if len(self.system_metrics) >= 5 else self.system_metrics
        ...
        return {
            ...
            "cpu": {
                ...
                "trend": cpu_trend,
                "status": "healthy" if latest.cpu_percent < 80 else "warning" if latest.cpu_percent < 90 else "critical"
            },
            "memory": {
                ...
                "trend": memory_trend,
                "status": "healthy" if latest.memory_percent < 80 else "warning" if latest.memory_percent < 90 else "critical"
            },
            "disk": {
                "percent": latest.disk_percent,
                "used_gb": latest.disk_used_gb,
                "total_gb": latest.disk_total_gb,
                "status": "healthy" if latest.disk_percent < 80 else "warning" if latest.disk_percent < 90 else "critical"
            },
            "load": {
                "1m": latest.load_avg_1m,
                "5m": latest.load_avg_5m,
                "15m": latest.load_avg_15m,
                "status": "healthy" if latest.load_avg_1m < 1.0 else "warning" if latest.load_avg_1m < 2.0 else "critical"
            },
            "uptime": {
    ...
    async def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        healthy_services = len([s for s in service_health.values() if s.status == ServiceStatus.HEALTHY])
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            stats.update({
                ...
                "enabled_monitors": len([m for m in self.monitor_configs.values() if m.enabled]),
    ...
    async def _collect_system_metrics(self) -> None:
        ...
        try:
            ...
            await self.collect_metric("system_memory", "memory_used_mb", memory.used / 1024 / 1024, "MB")
            await self.collect_metric("system_memory", "memory_total_mb", memory.total / 1024 / 1024, "MB")
            
            # 磁盘使用
            disk = psutil.disk_usage('/')
            await self.collect_metric("system_disk", "disk_percent", disk.percent, "%")
            await self.collect_metric("system_disk", "disk_used_gb", disk.used / 1024 / 1024 / 1024, "GB")
            await self.collect_metric("system_disk", "disk_total_gb", disk.total / 1024 / 1024 / 1024, "GB")
            ...
            if self._last_network_stats and self._last_network_time:
                ...
                await self.collect_metric("system_network", "network_sent_kbps", sent_rate / 1024 * 8, "kbps")
                await self.collect_metric("system_network", "network_recv_kbps", recv_rate / 1024 * 8, "kbps")
            ...
            logger.debug(f"收集系统指标: CPU={cpu_percent}%, 内存={memory.percent}%, 磁盘={disk.percent}%")
    ...
    async def _create_alert(self, monitor_id: str, alert_level: AlertLevel,
        ...
        for alert in existing_alerts:
            if (alert.metric_name == metric_name and 
                ...
                abs(alert.metric_value - metric_value) < 0.1 and
...
async def example_usage():
    ...
    try:
        ...
        print(f"仪表板数据: 系统健康={dashboard.get('system', {}).get('cpu', {}).get('status', 'unknown')}")
