...
class TestAPIServer:
    ...
    async def test_api_response(self):
        ...
        success_response = APIResponse(
            status_code=200, data={"result": "success"}, message="操作成功", request_id="req_123"
        )
        ...
        error_response = APIResponse(
            status_code=400, data=None, error="参数错误", message="请求参数无效"
        )
    ...
    async def test_rate_limit(self):
        ...
        custom_limit = RateLimit(
            requests_per_minute=100, requests_per_hour=5000, requests_per_day=50000, burst_size=20
        )
    ...
    async def test_error_handling(self, api_server):
        ...
        # 测试错误响应
        error_response = APIResponse(
            status_code=500, data=None, error="内部服务器错误", message="系统异常"
        )
