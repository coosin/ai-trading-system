...
class TestLogManager:
    ...
    async def test_query_logs_by_level(self, log_manager):
        """测试按级别查询日志"""
        # 记录不同级别的日志
        await log_manager.info(source=LogSource.SYSTEM, message="INFO日志1", module="test")

        await log_manager.warning(source=LogSource.SYSTEM, message="WARNING日志1", module="test")

        await log_manager.error(source=LogSource.SYSTEM, message="ERROR日志1", module="test")

        await log_manager.info(source=LogSource.SYSTEM, message="INFO日志2", module="test")
    ...
    async def test_query_logs_by_source(self, log_manager):
        """测试按来源查询日志"""
        # 记录不同来源的日志
        await log_manager.info(source=LogSource.SYSTEM, message="系统日志", module="system")

        await log_manager.info(source=LogSource.TRADING, message="交易日志", module="trading")

        await log_manager.info(source=LogSource.API, message="API日志", module="api")
    ...
    async def test_query_logs_by_time_range(self, log_manager):
        """测试按时间范围查询日志"""
        # 记录一个带时间的日志
        await log_manager.info(source=LogSource.SYSTEM, message="时间测试日志", module="time_test")
    ...
    async def test_query_logs_by_module(self, log_manager):
        """测试按模块查询日志"""
        # 记录不同模块的日志
        await log_manager.info(source=LogSource.SYSTEM, message="模块1日志", module="module_1")

        await log_manager.info(source=LogSource.SYSTEM, message="模块2日志", module="module_2")

        await log_manager.info(
            source=LogSource.SYSTEM, message="模块1另一个日志", module="module_1"
        )
    ...
    async def test_query_logs_by_tags(self, log_manager):
        """测试按标签查询日志"""
        # 记录不同标签的日志
        await log_manager.info(source=LogSource.SYSTEM, message="标签测试1", tags=["tag1", "tag2"])

        await log_manager.info(source=LogSource.SYSTEM, message="标签测试2", tags=["tag2", "tag3"])

        await log_manager.info(source=LogSource.SYSTEM, message="标签测试3", tags=["tag1", "tag3"])
    ...
    async def test_query_logs_with_pagination(self, log_manager):
        ...
        for i in range(20):
            await log_manager.info(
                source=LogSource.SYSTEM, message=f"分页测试日志 {i:02d}", data={"index": i}
            )
    ...
    async def test_get_log_statistics(self, log_manager):
        ...
        for i in range(10):
            ...
            await log_manager.log(
                level=level, source=source, message=f"统计测试日志 {i}", module="stats_test"
            )
    ...
    async def test_get_log_statistics_with_time_range(self, log_manager):
        ...
        # 记录当前时间的日志
        await log_manager.info(
            source=LogSource.SYSTEM, message="当前时间日志", module="time_range_test"
        )
    ...
    async def test_get_memory_buffer(self, log_manager):
        ...
        # 记录一些日志
        for i in range(5):
            await log_manager.info(source=LogSource.SYSTEM, message=f"缓冲区测试 {i}")
    ...
    async def test_clear_memory_buffer(self, log_manager):
        ...
        # 记录一些日志
        for i in range(3):
            await log_manager.info(source=LogSource.SYSTEM, message=f"清空测试 {i}")
    ...
    async def test_export_logs_json(self, log_manager):
        ...
        for i in range(3):
            await log_manager.info(
                source=LogSource.SYSTEM, message=f"导出测试 {i}", data={"export_index": i}
            )
    ...
    async def test_export_logs_csv(self, log_manager):
        ...
        for i in range(2):
            await log_manager.info(
                source=LogSource.SYSTEM, message=f"CSV导出测试 {i}", data={"csv_test": True}
            )
