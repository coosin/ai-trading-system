...
from src.modules.core.monitor_manager import (
    MonitorManager, MonitorConfig, MonitorMetric, MonitorAlert, ServiceHealth,
    MonitorType, AlertLevel, ServiceStatus, SystemMetrics
)
...
class TestMonitorManager:
    ...
    async def test_get_metrics(self, monitor_manager, sample_monitor_config):
        ...
        recent_metrics = await monitor_manager.get_metrics(
            start_time=hour_ago,
            end_time=now
        )
