...
class TestDatabaseManager:
    ...
    async def db_manager(self):
        """创建测试用的数据库管理器"""
        # 使用内存数据库进行测试
        config = DatabaseConfig(type=DatabaseType.MEMORY, database=":memory:", echo=False)
        ...
        class MockAsyncSession:
            ...
            async def execute(self, query, params=None):
                class MockResult:
                    ...
                    def _mapping(self):
                        return type(
                            "obj", (object,), {"items": lambda: [("id", 1), ("name", "test")]}
                        )()
    ...
    async def test_crud_operations(self, db_manager):
        ...
        # 测试更新
        try:
            count = await db_manager.update("test_table", {"value": 456}, {"name": "test"})
