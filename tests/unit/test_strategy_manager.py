...
class TestStrategyManager:
    ...
    async def test_register_strategy_class(self, strategy_manager):
        ...
        # 注册策略类
        success = await strategy_manager.register_strategy_class(
            "test_strategy_class", TestStrategy
        )
    ...
    @pytest.mark.asyncio
    async def test_get_strategy_instances_with_filters(
        self, strategy_manager, sample_strategy_config
    ):
        """测试带过滤器的策略实例获取"""
        # 加载配置并创建多个实例
        config_dict = sample_strategy_config.to_dict()
        ...
        # 按策略ID过滤
        strategy_instances = await strategy_manager.get_strategy_instances(
            strategy_id="test_strategy_1"
        )
        ...
        # 按状态过滤
        created_instances = await strategy_manager.get_strategy_instances(
